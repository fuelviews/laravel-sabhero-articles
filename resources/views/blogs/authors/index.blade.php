                ...
                @foreach ($authors as $user)
                    <div class="bg-white p-6 rounded-lg shadow">
                        <a href="{{ route('sabhero-blog.author.show', ['author' => $user->author->slug]) }}">
                            <img
                                srcset="{{ $user->author->getFirstMedia('avatar')->getSrcset() }}"
                                src="{{ $user->author->getFirstMedia('avatar')->getUrl() }}"
                                class="h-48 w-48 rounded-full mx-auto hover:opacity-65" alt="{{ $user->name }}">
